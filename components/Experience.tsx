...
const tags = ['Shopify', 'Liquid', 'Tailwind CSS', 'JavaScript', 'HTML/CSS'];

const responsibilities = [
  'Developing and customizing Shopify storefronts using Liquid and Shopify Themes.',
  'Building custom sections and apps to extend store functionality.',
  'Optimizing theme performance and SEO to improve conversion rates.',
  'Aligning landing page experiences with advertising campaign goals.',
];

function Experience() {
  return (
    <section id="experience" className="mt-16">
      <h3 className="mb-8 mt-8 text-2xl font-semibold">Experience</h3>

      <div className="group relative">
        <div
          className="absolute -inset-1 rounded-3xl bg-gradient-to-br from-teal-500/30 via-emerald-500/15 to-transparent opacity-70 blur-xl transition-opacity duration-300 group-hover:opacity-100"
          aria-hidden="true"
        />
        <div className="relative h-full rounded-3xl bg-gradient-to-br from-teal-500/40 via-slate-700/40 to-slate-800/40 p-px transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-teal-500/20">
          <div className="relative flex h-full flex-col rounded-[calc(1.5rem-1px)] bg-slate-950/95 p-6">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-white p-2 shadow-lg shadow-teal-500/25">
                  <svg
                ...
                <div>
                  <h4 className="text-lg font-semibold">Shopify Developer</h4>
                  <p className="text-sm text-teal-400">Scale Up Ads Agency</p>
                </div>
              </div>
              <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-teal-500/30 bg-teal-500/10 px-2.5 py-1 text-xs font-medium text-teal-300">
                <Calendar className="size-3" />
                Jun 2025 — Present
              </span>
            </div>

            <div className="my-4 h-px bg-gradient-to-r from-teal-500/40 via-slate-700/40 to-transparent" />

            <div className="flex items-start gap-2">
              <Sparkles className="mt-0.5 size-4 shrink-0 text-teal-400" />
              <p className="text-sm leading-relaxed text-slate-300">
                Built{' '}
                <span className="font-semibold text-teal-400">
                  high-converting Shopify storefronts
                </span>{' '}
                that directly support paid advertising campaigns.
              </p>
            </div>
            ...
            <div className="mt-auto flex flex-wrap gap-2 pt-5">
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-slate-700/60 bg-slate-900 px-2.5 py-1 text-xs text-slate-300"
                >
                  {tag}
                </span>
              ))}
